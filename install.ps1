<#
.SYNOPSIS
    One-click Windows installer for LocalAiLab — Multipurpose Local AI Assistant.
.DESCRIPTION
...
if (-not $NoShortcut) {
    ...
    if (Test-Path $runBat) {
        $logoJpg = Join-Path $targetDir "image\logo.jpg"
        if (-not (Test-Path $logoJpg)) { $logoJpg = Join-Path $targetDir "logo.jpg" }
        $logoIco = Join-Path $targetDir "image\logo.ico"
